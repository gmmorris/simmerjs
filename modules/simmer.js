...
export default function(window, QueryEngine, undefined) {
    ...
    while (!parser.finished() && !selectorState.verified) {
      try {
        ...
        if (
          ...
          selectorState.verified = analyzeSelectorState(
            element,
            selectorState,
    ...
    for (index = 0; index < hierarchy.length && !state.verified; index += 1) {
      ...
      if (currentID && isUniqueElementID($DOM, currentID)) {
        ...
        if (index === 0) {
          ...
          if (
            analyzeSelectorState(hierarchy[0], state, config.selectorMaxLength)
          ) {
   ...
   * @param {object} state. The current calculated CSS selector
   */
  parsingMethods.addMethod(
    function(hierarchy, state, config) {
      ...
      while (hierarchyIndex < hierarchy.length && !state.verified) {
        ...
        if (siblings.length !== 0) {
          ...
          if (!hasUniqueClassOrTag) {
            state.stack[hierarchyIndex].push(
              ':nth-child(' + indexOfElement + ')'
            )

            // Verify the selector as we don't want to go on and parse the parent's siblings
            // if we don't have to!
            state.verified = analyzeSelectorState(
              hierarchy[0],
              state,
              config.selectorMaxLength
            )
